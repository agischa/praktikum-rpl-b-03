US-01 As a User, I want to input income into a selected account, so that my account balance increases correctly. 
Acceptance Criteria: 
...
US-02 As a User, I want to see a summary of my total balance from all accounts, so that I can monitor my financial condition. 
Acceptance Criteria: 
...
US-03 As a User, I want to filter transaction history by date, so that I can review financial activity. 
Acceptance Criteria: 
...
US-04 As a User, I want to edit transaction data, so that incorrect inputs can be fixed. 
Acceptance Criteria: 
...
US-05 As a User, I want to categorize my expenses and view them in a chart, so that I understand my spending patterns. 
Acceptance Criteria: 
...
US-06 As a User, I want to allocate balance into Brankas, so that I can save money for specific goals. 
Acceptance Criteria: 
...
US-07 As a User, I want Brankas balance to be isolated, so that it is not used in daily transactions. 
Acceptance Criteria: 
...
US-08 As a User, I want to see saving progress, so that I know how far I am from my target. 
Acceptance Criteria: 
...
US-09 As a User, I want to record daily expenses from a selected account, so that my balance updates correctly. 
Acceptance Criteria: 
...
US-10 As a User, I want to manage multiple accounts (e.g., Cash, BCA), so that I can separate my money sources. 
Acceptance Criteria: 
...
US-11 As a New User, I want to continue using the app as a guest without creating an account, so that I can set up my first wallet immediately. 
Acceptance Criteria: 
Given: User is on the onboarding screen. 
When: User selects the "Continue as Guest" option and confirms the local storage warning. 
Then: System bypasses cloud login and navigates directly to the wallet setup page.

US-12 As a User, I want to transfer balance between accounts, so that I can manage money distribution without affecting expense stats. 
Acceptance Criteria: 
Given: User opens the transfer menu. 
When: User inputs nominal, source account, destination account, and saves. 
Then: System deducts from the source, adds to the destination, and records it specifically as a Transfer.

US-13 As a Registered User, I want to log in with my email and password, so that my transaction history is synced securely to the cloud. 
Acceptance Criteria: 
Given: User is on the cloud sync settings page. 
When: User enters valid email and password credentials and logs in. 
Then: System authenticates via JWT and automatically backs up or restores data from the database.
