...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-gray-400">Catat semua sumber pemasukanmu</p>
    </div>
    <button onclick="openModal('addModal')"
        class="flex items-center gap-2 px-5 py-2.5 bg-[#D1E7DD] hover:bg-green-200 text-green-700 font-semibold rounded-2xl transition text-sm">
...
<div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-6">
    <form method="GET" action="{{ route('incomes.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <input type="text" name="search" value="{{ request('search') }}"
            ...
            class="px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300">

        <select name="category" class="px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300">
        ...
        <div class="flex gap-2">
            <input type="date" name="date_to" value="{{ request('date_to') }}"
                class="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300">
            <button type="submit" class="px-4 py-2.5 bg-[#D1E7DD] text-green-700 rounded-xl text-sm font-medium hover:bg-green-200 transition">
                <i class="fa-solid fa-filter"></i>
            </button>
            <a href="{{ route('incomes.index') }}" class="px-4 py-2.5 bg-gray-100 text-gray-500 rounded-xl text-sm font-medium hover:bg-gray-200 transition">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-2">
                                <button onclick="openEditModal({{ $income->id }}, '{{ $income->date }}', {{ $income->amount }}, '{{ $income->category }}', {{ $income->wallet_id ?? 'null' }}, '{{ addslashes($income->description) }}')"                                
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-6 py-16 text-center">
                        ...
                        <p class="text-gray-400 text-sm">Belum ada data pemasukan</p>
                        <button onclick="openModal('addModal')" class="mt-3 text-green-600 text-sm hover:underline">
                            + Tambah sekarang
                        </button>
                    </td>
                </tr>
        ...
        <form method="POST" action="{{ route('incomes.store') }}" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="date" name="date" required
                    class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300">
            </div>
            <div>
            ...
            <div>
                ...
                <textarea name="description" rows="2" placeholder="Keterangan tambahan..."
                    class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-green-300 resize-none"></textarea>
            </div>
            <button type="submit"
                class="w-full py-3 bg-[#D1E7DD] hover:bg-green-200 text-green-700 font-semibold rounded-2xl transition text-sm">
                Simpan Pemasukan
            </button>
        </form>
    </div>
...
<div id="editModal" class="fixed inset-0 bg-black/40 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-xl w-full max-w-md">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
            <h3 class="font-semibold text-gray-800">Edit Pemasukan</h3>
...
<script>
    ...
    document.getElementById('editForm').action = '/incomes/' + id;
